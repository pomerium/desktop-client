...
const waitForRoute = async (page: Page, matcher: string | RegExp, timeout = 15000) => {
...
const getAppWindow = async (electronApp: ElectronApplication) => {
  for (let attempt = 0; attempt < 60; attempt += 1) {
    ...
    for (const candidate of pages) {
      try {
        const bodyText = await candidate.evaluate(() => document.body?.innerText || "");
        if (
...
const expectListenerAcceptsConnection = async (port: number, payload: string) => {
  return new Promise<void>((resolve, reject) => {
    ...
    socket.on("close", () => {
      ...
      if (!settled) {
        finish(() => reject(new Error(`listener ${port} closed before accepting a client`)));
      }
    });
  });
};
...
const expectPortClosed = async (port: number) => {
  ...
  throw new Error(`listener on ${port} stayed open after disconnect: ${lastErrors.join(" | ")}`);
};
...
async function main() {
  ...
  const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "desktop-client-acceptance-"));
  ...
  try {
    ...
    await page.getByLabel("Destination").fill(`127.0.0.1:${echoServer.port}`);
    await page.getByLabel("Local Address").fill(`127.0.0.1:${localPort}`);
    ...
    await page.getByRole("dialog").getByRole("heading", { name: "Export", exact: true }).waitFor({
      state: "visible",
    });
    ...
    await page.getByRole("button", { name: "toggle listeners for Untagged" }).click();
    await page.getByText(editedConnectionName, { exact: true }).waitFor({
      state: "visible",
      timeout: 15000,
    });
    ...
    await page.getByText(`Listening on 127.0.0.1:${localPort}`, { exact: true }).waitFor({
      state: "visible",
      timeout: 15000,
    });
    await expectListenerAcceptsConnection(localPort, `${probePayload}-imported`);
