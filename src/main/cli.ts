...
export async function start(sentryDSN: string): Promise<CLI> {
  ...
  const configClient = new ConfigClient(grpcAddress, ChannelCredentials.createInsecure());
  const listenerClient = new ListenerClient(grpcAddress, ChannelCredentials.createInsecure());
